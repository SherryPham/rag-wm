...
def settingtwo(cfg: RagWmConfig, data_dir: str, rng: random.Random, aux_model: HfModel, sim_model: SentenceTransformer) -> None:
    ...
    # Docs per set. Defaults to 200 (paper-scale); override with env var WARD_N for a quick
    # smoke test, e.g. WARD_N=5, since the local reasoning model is slow per generation.
    N = int(os.environ.get("WARD_N", 200))
