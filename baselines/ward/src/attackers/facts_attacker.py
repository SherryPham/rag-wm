...
def _local_generate(model: Any, sysprompt: str, prompts: List[str]) -> List[str]:
    """Generate text with a local HfModel using an APIModel-style (sysprompt, prompts) call.

    HfModel has no dedicated system-prompt slot (its chat template is user-only), so we
    fold the system instruction into each user prompt. We generate one prompt at a time to
    keep memory bounded -- documents can be long and the model may be a small local one.
    """
    responses: List[str] = []
    for p in prompts:
        completions, _cost = model.generate([f"{sysprompt}\n\n{p}"])
        responses.append(completions[0])
    return responses
...
class FactsAttacker:
    ...
    def __init__(
        ...
        # Probe questions used to be generated by GPT-4o (paid). To run fully local/free we
        # reuse the RAG system's already-loaded model at question-generation time instead.
    ...
    def _get_signals(
        ...
        # Generate any missing probe questions with the local RAG model (was GPT-4o) and cache them
        new_questions = _local_generate(rag_system.model, sysprompt, [q[1] for q in gpt_queries])
        for (idd, _), resp in zip(gpt_queries, new_questions):
