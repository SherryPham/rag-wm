...
class APIModel:
    def __init__(self, model_name: str) -> None:
        ...
        elif model_name == "qwen2.5-ollama" or model_name.startswith("ollama:"):
            # Local Ollama via its OpenAI-compatible endpoint. Reuses the OpenAI code path
            # in _query_api (chat.completions.create). The served tag is taken from
            # "ollama:<tag>" if given, else $OLLAMA_MODEL, else qwen2.5:3b.
            self.provider = "openai"
            base_url = os.environ.get("OLLAMA_BASE_URL", "http://localhost:11434/v1")
            self.client = openai.OpenAI(
                base_url=base_url, api_key=os.environ.get("OLLAMA_API_KEY", "ollama")
            )
            self.model_name = model_name
            if model_name.startswith("ollama:"):
                self.full_model_name = model_name.split("ollama:", 1)[1]
            else:
                self.full_model_name = os.environ.get("OLLAMA_MODEL", "qwen2.5:3b")
            return
        else:
