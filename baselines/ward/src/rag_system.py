...
class RagSystem:
    ...
    def __init__(
        ...
        # The vector store is only queried in the real_retrieval path (see generate()).
        # Milvus Lite (the embedded "./experiment.db" backend) ships wheels for Linux/macOS
        # only, so skip building it entirely when we're not doing real retrieval -- this keeps
        # the non-retrieval path (which uses "perfect" retrieval from the corpus) Windows-friendly.
        self.vectordb = None
        if self.real_retrieval:
